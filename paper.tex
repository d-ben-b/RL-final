...
$368.5$ average survival steps. This figure warrants caution. DQN changes lanes
\emph{far} more than any other model on average ($155$ per episode), but the
count is highly seed- and episode-dependent: across individual rollouts it ranges
from $0$ (driving straight in lane) to nearly $400$ (switching lanes almost every
step) while still surviving. Because the aggressive \emph{training} reward
\emph{positively} rewards lane changes ($+0.2$; Table~\ref{tab:reward}), part of
DQN's low crash rate may reflect a degenerate ``swerve-to-survive'' strategy that
exploits this incentive rather than principled obstacle avoidance; we flag this
as an open caveat. A supplementary \code{highway-v0} video accompanies this
comparison, showing PPO+Attention colliding while DQN survives on identical
traffic. In contrast, \textbf{PPO+Attention and PPO+MLP exhibit
...
than principled avoidance. A second supplementary \code{merge-v0} video shows DQN
completing the merge while PPO collides on identical traffic. With
